...
let templateName = "Viper Screen.xctemplate"
...
func moveTemplate(){
    ...
    print("\(destinationPath)/\(templateName)")
    do {
        if !fileManager.fileExists(atPath:"\(destinationPath)/\(templateName)"){
            ...
            try _ = fileManager.replaceItemAt(URL(fileURLWithPath:"\(destinationPath)/\(templateName)"), withItemAt: URL(fileURLWithPath:templateName), options: .withoutDeletingBackupItem)
            // TODO: Fix this dirty hack
            try fileManager.copyItem(atPath: "\(destinationPath)/\(templateName)", toPath: templateName)
